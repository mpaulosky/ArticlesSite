...
public class HomePage : BasePage
{
	...
	public async Task<bool> HasWelcomeMessageAsync()
	{
		try
		{
			return await _welcomeMessage.IsVisibleAsync(new LocatorIsVisibleOptions { Timeout = 5000 });
		}
		catch
		{
			return false;
	...
	/// <summary>
	/// Check if a recent articles section is visible
	/// </summary>
	[Obsolete]
	public async Task<bool> HasRecentArticlesAsync()
	{
		try
		{
			return await _recentArticles.IsVisibleAsync(new LocatorIsVisibleOptions { Timeout = 5000 });
		}
		catch
		{
			return false;
