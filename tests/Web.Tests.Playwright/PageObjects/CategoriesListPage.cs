...
public class CategoriesListPage : BasePage
{
    ...
    public async Task<bool> HasCategoriesListAsync()
    {
        try
        {
            return await _categoriesList.IsVisibleAsync(new() { Timeout = 5000 });
        }
        catch
        {
            return false;
    ...
    public async Task<bool> HasCreateButtonAsync()
    {
        try
        {
            return await _createButton.IsVisibleAsync(new() { Timeout = 5000 });
        }
        catch
        {
            return false;
    ...
    {
        if (await _searchInput.IsVisibleAsync(new() { Timeout = 2000 }))
        {
    ...
    public async Task<bool> HasNoCategoriesMessageAsync()
    {
        try
        {
            return await _noCategoriesMessage.IsVisibleAsync(new() { Timeout = 5000 });
        }
        catch
        {
            return false;
