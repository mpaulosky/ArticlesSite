...
public class ErrorPage : BasePage
{
    ...
    public async Task<bool> IsErrorPageDisplayedAsync()
    {
        try
        {
            return await _errorHeading.IsVisibleAsync(new() { Timeout = 5000 }) || 
                   await _errorCode.IsVisibleAsync(new() { Timeout = 5000 });
        }
        catch
        {
            return false;
    ...
    public async Task<bool> HasHomeLinkAsync()
    {
        try
        {
            return await _homeLink.IsVisibleAsync(new() { Timeout = 5000 });
        }
        catch
        {
            return false;
    ...
    {
        if (await _backButton.IsVisibleAsync(new() { Timeout = 2000 }))
        {
