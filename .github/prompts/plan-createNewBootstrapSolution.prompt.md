...
# PRD Plan: Create New Bootstrap-Based Blog Solution

Repository: New Project (similar to mpaulosky/ArticlesSite)
Project focus: Greenfield .NET 10 Blazor Server + MongoDB blog application
Goal: Build a modern, cloud-native blog platform with Bootstrap styling (CDN), Auth0 authentication, CQRS architecture, and comprehensive testing—without E2E tests.
...
│   │   ├── wwwroot/
│   │   │   ├── css/
│   │   │   │   └── app.css         # Bootstrap + custom overrides
│   │   │   ├── js/
│   │   │   │   └── theme-toggle.js
│   │   │   └── img/
...
- **Delivery**: CDN (Bootstrap CDN) for CSS + Bundle JS
  - CSS: `<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.x/dist/css/bootstrap.min.css" rel="stylesheet">`
  - JS: `<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.x/dist/js/bootstrap.bundle.min.js"></script>`
  - Rationale: No npm/Node toolchain required; simplest path for greenfield; CSP friendly with SRI hashes
...
- **Auth0 Integration**:
  ...
  - Auth State Provider for Blazor context
- **Protected Pages**: Redirect to login if not authenticated
- **User Context**: Available in components via `@context.User` or injected `AuthenticationStateProvider`
...
- `Markdig` - Markdown parsing for article content
...
1. Create solution: `dotnet new sln -n NewBlogApp`
2. Create projects:
...
1. Add Bootstrap CDN link to `App.razor` layout (or `_Host.cshtml` if using server-side rendering)
2. Create `wwwroot/js/theme-toggle.js` for early theme initialization
3. Create `wwwroot/css/app.css` for custom overrides
...
1. **Should we scaffold the solution now or finalize this plan first?**
2. **Bootstrap CDN provider preference: jsDelivr, unpkg, or official Bootstrap CDN?**
3. **Markdown editor library: stick with Markdig (server-side) or add client-side library (e.g., EasyMDE)?**
4. **File upload destination: Azure Blob Storage, AWS S3, or local wwwroot?**
5. **Initial feature set: just Articles & Categories, or include Pages, Tags, Comments from Day 1?**
6. **Database seeding: auto-seed on startup or manual import tool?**
