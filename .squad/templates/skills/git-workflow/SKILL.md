---
name: "git-workflow"
description: "Squad branching model: main-first workflow with optional worktrees"
...
Squad uses a protected-branch model. **All feature work starts from `main` by default.**
...
| `dev` | Optional integration branch (protected, no direct pushes) | Optional preview channel |
...
- ❌ Branching directly on `main` or `dev` (always create `squad/{issue}-{slug}`)
- ❌ Direct pushes to `main` or `dev` (use PR flow)
...
- Teams may still run `dev`/`insiders` as channels.
- This standard only fixes contribution flow: start from `main` issue branches, review, merge, cleanup.
