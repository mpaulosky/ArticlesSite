...
// Response capture middleware moved to after the app is built to avoid using 'app' before declaration.
...
builder.Services.Configure<ConcurrencyOptions>(builder.Configuration.GetSection("ConcurrencyOptions"));

// Register a centralized Polly policy for concurrency retries as a strongly typed IAsyncPolicy<Result<Article>>
builder.Services.AddSingleton<IAsyncPolicy<Result<Article>>>(sp =>
{
...
app.Use(async (context, next) =>
{
	...
	try
	{
		...
		// If non-success, attempt to read the body
...
app.Use(async (context, next) =>
{
	...
	try
	{
		...
		// Write to the current directory for local runs
		try
		{
			var logPath = Path.Combine(Directory.GetCurrentDirectory(), "articlesite-integration.log");
		...
		try
		{
			var tempPath = Path.Combine(Path.GetTempPath(), "articlesite-integration.log");
...
app.MapPut("/api/articles/{id}", async (HttpContext httpContext, string id, ArticleDto dto, EditArticle.IEditArticleHandler handler, IConfiguration config) =>
{
	...
	{
		if (result.Details is ConcurrencyConflictInfo conflict)
		{
