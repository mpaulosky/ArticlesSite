...
) : ICategoryRepository
{
	...
	public async Task<Result<Category>> UpdateCategory(Category category)
	{
		try
		{
			...
			var replacement = BsonSerializer.Deserialize<Category>(category.ToBsonDocument());
			...
			var details = new ConcurrencyConflictInfo(server.Version, null, changed);
			return Result.Fail<Category>("Concurrency conflict: category was modified by another process", ResultErrorCode.Concurrency, details);
		}
		catch (Exception ex)
		{
