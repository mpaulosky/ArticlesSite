...
public static class EditCategory
{
	...
	public class Handler : IEditCategoryHandler
	{
		...
		public Handler(ICategoryRepository repository, ILogger<Handler>? logger, IValidator<CategoryDto>? validator, IOptions<ConcurrencyOptions>? concurrencyOptions = null, IAsyncPolicy<Result<Category>>? concurrencyPolicy = null)
		{
			...
			_concurrencyPolicy = concurrencyPolicy ?? ConcurrencyPolicies.CreatePolicy<Category>(_concurrencyOptions);
		}
		...
		public async Task<Result<CategoryDto>> HandleAsync(CategoryDto dto)
		{
			...
			var concreteValidation = new CategoryDtoValidator().Validate(dto);
			ValidationResult injectedValidation = null!;
			if (_validator != null)
			{
			...
			if (context.TryGetValue("terminalFailure", out var term) && term is Result<Category> terminal && terminal.Failure)
			{
				...
				{
					return Result.Fail<CategoryDto>(terminal.Error ?? "Concurrency conflict: category was modified by another process", ResultErrorCode.Concurrency, terminal.Details);
				}
			...
			if (policyResult.Failure)
			{
				...
				{
					return Result.Fail<CategoryDto>(policyResult.Error ?? "Concurrency conflict: category was modified by another process", ResultErrorCode.Concurrency, policyResult.Details);
				}
