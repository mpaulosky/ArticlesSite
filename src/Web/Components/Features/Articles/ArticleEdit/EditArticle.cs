...
public static class EditArticle
{
	...
	public class Handler : IEditArticleHandler
	{
		...
		private readonly IAsyncPolicy<Result<Article>> _concurrencyPolicy;
		private readonly IMetricsPublisher _metrics;

		public Handler(IArticleRepository repository, ILogger<Handler> logger, IValidator<ArticleDto>? validator, IOptions<ConcurrencyOptions> concurrencyOptions, IAsyncPolicy<Result<Article>>? concurrencyPolicy = null, IMetricsPublisher? metrics = null)
		{
			this.repository = repository ?? throw new ArgumentNullException(nameof(repository));
			this.logger = logger ?? NullLogger<Handler>.Instance;
			this.validator = validator;
			_concurrencyOptions = concurrencyOptions?.Value ?? new ConcurrencyOptions();
			_concurrencyPolicy = concurrencyPolicy ?? ConcurrencyPolicies.CreatePolicy(_concurrencyOptions);
			_metrics = metrics ?? new NoOpMetricsPublisher();
		}
		...
		public Handler(IArticleRepository repository, ILogger<Handler> logger, IValidator<ArticleDto>? validator)
				: this(repository, logger, validator, Options.Create(new ConcurrencyOptions()))
		{
		}
		...
		public async Task<Result<ArticleDto>> HandleAsync(ArticleDto? dto)
		{
			...
			// Run validator if provided; always ensure concrete validator rules are applied
			var concreteArticleValidator = new ArticleDtoValidator();
			ValidationResult concreteValidation = concreteArticleValidator.Validate(dto) ?? new ValidationResult();
			...
			if (context.TryGetValue("terminalFailure", out var term) && term is Result<Article> terminal && terminal.Failure)
			{
				...
				if (terminal.ErrorCode == ResultErrorCode.Concurrency)
				{
					return Result.Fail<ArticleDto>(terminal.Error ?? "Concurrency conflict: article was modified by another process", ResultErrorCode.Concurrency, terminal.Details);
				}
			...
			if (policyResult.Failure)
			{
				...
				if (policyResult.ErrorCode == ResultErrorCode.Concurrency)
				{
					return Result.Fail<ArticleDto>(policyResult.Error ?? "Concurrency conflict: article was modified by another process", ResultErrorCode.Concurrency, policyResult.Details);
				}
